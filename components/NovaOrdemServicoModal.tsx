              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-inner">
                {[
                  ...
                  <div key={field.name} className={field.full ? 'md:col-span-2' : ''}>
                    ...
                    {field.type === 'select' ? (
                      <select title={field.label} name={field.name} value={(formData as any)[field.name]} onChange={handleChange} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500/20">
