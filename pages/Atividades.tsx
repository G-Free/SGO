...
const NovaAtividadeModal = lazy(
  () => import("../components/NovaAtividadeModal"),
);
const DetalheAtividadeModal = lazy(
  () => import("../components/DetalheAtividadeModal"),
);
...
const mockActivities = [
  {
    ...
    planoAcaoId: "PA-02",
    objectives: [
      "Verificar cumprimento das normas de pesca",
      "Inspecionar embarcações",
    ],
    source: "list",
  },
  {
  ...
  {
    id: "ATV-003",
    title: "Upgrade de Servidores Centrais",
    responsible: "Gestor 1",
  ...
  {
    id: "ATV-006",
    title: "Patrulha Porto Pesqueiro",
...
const mockActivityReports = [
  {
    id: "RAT-001",
    activityId: "ATV-005",
    title: "Relatório de Backup Trimestral",
    date: "2024-07-08",
    status: "Validado",
    author: "Técnico A",
    province: "Luanda",
    feedback: "Documentação impecável.",
  },
  {
    id: "RAT-002",
    activityId: "ATV-001",
    title: "Relatório Fiscalização Costa Norte",
    date: "2024-07-05",
    status: "Submetido",
  ...
  },
  {
    ...
    date: "2024-07-28",
    status: "Devolvido",
    author: "Técnico Operação",
    province: "Cabinda",
    feedback: "As coordenadas registradas divergem do plano original.",
  },
];
...
const mockActivityReportsForApproval = [
  {
    id: "RAT-004",
    activityId: "ATV-001",
    title: "Relatório Final Costa Norte",
    date: "2024-07-05",
    status: "Pendente",
    author: "Cmdt. Manuel Santos",
    province: "Cabinda",
  },
];
...
const CalendarView: React.FC<{
  activities: any[];
  onViewDetails: (act: any) => void;
}> = ({ activities, onViewDetails }) => {
  ...
  const calendarDays = Array.from({ length: firstDayOfMonth }, (_, i) => (
    <div
      key={`empty-${i}`}
      className="h-32 bg-slate-50/30 border border-slate-100"
    ></div>
  ));
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    calendarDays.push(
      <div
        key={day}
        className={`h-32 border border-slate-100 p-2 overflow-y-auto group transition-colors ${isToday ? "bg-blue-50/30 ring-1 ring-inset ring-blue-200" : "bg-white hover:bg-slate-50"}`}
      >
        <div className="flex justify-between items-start">
          <span
            className={`text-[10px] font-black ${isToday ? "text-blue-600" : "text-slate-300"} group-hover:text-blue-500`}
          >
            {day}
          </span>
        </div>
        <div className="space-y-1 mt-1">
          {dayActivities.map((act) => (
            <div
              ...
              className={`text-[8px] font-black uppercase p-1 rounded truncate cursor-pointer shadow-sm border ${statusColors[act.status] || "bg-slate-100"}`}
              title={act.title}
            >
              {act.title}
            </div>
    ...
    <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm animate-fadeIn">
      <div className="p-6 border-b flex flex-col sm:flex-row justify-between items-center bg-slate-50/50 gap-4">
        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center">
          <CalendarIcon className="mr-2 text-blue-600" size={18} /> Escala de
          Atividades
        </h3>
        <div className="flex items-center gap-3">
          <button
            onClick={handlePrevMonth}
            className="p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 shadow-sm"
          >
            <ChevronLeft size={16} />
          </button>
      ...
      <div className="grid grid-cols-7 gap-px bg-slate-100 border-b border-slate-100">
        {daysOfWeek.map((day) => (
          <div
            key={day}
            className="py-3 text-center text-[9px] font-black text-slate-400 uppercase tracking-widest"
          >
            {day}
          </div>
        ))}
...
const AtividadesPage: React.FC = () => {
  ...
  const handleSaveAtividade = (data: any) => {
    if (data.id && activities.some((a) => a.id === data.id)) {
      ...
      addNotification(
        "Nova atividade registada no sistema.",
        "success",
        "Sucesso",
      );
    }
  ...
  const filteredActivities = useMemo(() => {
    return activities.filter((act) => {
      ...
      const matchesPlan =
        selectedPlanId === "Todos" || act.planoAcaoId === selectedPlanId;
  ...
  const filteredReports = useMemo(() => {
    return mockActivityReports.filter((rep) => {
      ...
      const matchesStatus =
        reportStatusFilter === "Todos" || rep.status === reportStatusFilter;
      const matchesDate =
        (!dateFilter.start || rep.date >= dateFilter.start) &&
  ...
  const generateOfficialPDFBlob = async (report: any) => {
    ...
    doc.setTextColor(0);
    doc.setFontSize(14);
    doc.text("GRUPO OPERACIONAL MULTISSECTORIAL", pageWidth / 2, 75, {
      align: "center",
    });
    ...
    doc.setFontSize(16);
    doc.text("RELATÓRIO DA OPERAÇÃO NO ÂMBITO DO", pageWidth / 2, 112, {
      align: "center",
    });
    doc.setFontSize(14);
    const titleLines = doc.splitTextToSize(
      report.title?.toUpperCase() || "RELATÓRIO OPERACIONAL",
      130,
    );
    doc.text(titleLines, pageWidth / 2, 124, { align: "center" });
    doc.setFontSize(12);
    doc.text(
      `PROVÍNCIA: ${report.province?.toUpperCase() || "CABINDA"}`,
      pageWidth / 2,
      160,
      { align: "center" },
    );
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-bold text-sm flex items-center">
            <Activity className="h-4 w-4 mr-2 text-blue-600" /> Coordenação e
            Execução de Atividades em Território Nacional
          </p>
        </div>
        {canManage && (
          <button
            ...
            className="bg-blue-900 hover:bg-black text-white font-black text-xs uppercase tracking-widest py-3 px-6 rounded-xl flex items-center shadow-lg transition-all active:scale-95"
          >
      ...
      <div className="bg-slate-200/50 p-1.5 rounded-2xl flex flex-wrap gap-1 mb-8 w-fit shadow-inner">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center transition-all ${activeTab === tab.id ? "bg-white text-blue-900 shadow-md" : "text-slate-500 hover:text-slate-700"}`}
          >
            ...
            <input
              ...
              placeholder={
                activeTab === "relatorios"
                  ? "Pesquisar por título, ID ou autor..."
                  : "Pesquisar atividade..."
              }
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border-none rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-500 outline-none shadow-inner"
            />
          </div>
        </div>
        {activeTab === "relatorios" ? (
          <>
            <div className="w-full md:w-48">
              ...
              <select
                ...
                className="w-full bg-slate-50 border-none rounded-xl px-4 py-2 text-[10px] font-black uppercase text-slate-700 outline-none shadow-sm"
              >
            ...
            <div className="w-full md:w-auto flex gap-2">
              <div className="w-full md:w-32">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border-none rounded-xl px-3 py-2 text-[10px] font-bold text-slate-700 outline-none shadow-sm"
                />
              </div>
              <div className="w-full md:w-32">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">
          ...
          <div className="w-full md:w-64">
            ...
            <select
              title="Filtro de Plano Nacional"
      ...
      <div className="min-h-[500px]">
                  ...
                  {filteredActivities.map((act) => (
                    ...
                    >
                      <td className="px-6 py-5">
                        ...
                        <div className="text-[9px] text-blue-600 font-black uppercase mt-0.5 tracking-tighter">
                          ID: {act.id}
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-center text-xs font-bold text-slate-600 uppercase tracking-tighter">
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center text-xs font-black text-slate-500 uppercase tracking-tighter">
                          <MapPin size={14} className="mr-2 text-rose-500" />{" "}
                          {act.province}
                        </div>
                      </td>
                      <td className="px-6 py-5">
                      ...
                      <td className="px-6 py-5 text-center">
                        <span
                          className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${statusColors[act.status] || "bg-slate-100"}`}
                        >
                          {act.status}
                        </span>
                      </td>
        ...
        {activeTab === "calendario" && !isTecnicoCentral && (
          <CalendarView
        ...
        {activeTab === "relatorios" && (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden animate-fadeIn">
            <div className="p-6 border-b bg-slate-50/50 flex justify-between items-center">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center">
                <FileText className="mr-2 text-indigo-600" size={18} /> Central
                de Relatórios de Atividade
              </h3>
              {isGenerating && (
                <div className="flex items-center gap-2 text-blue-600 font-bold text-xs animate-pulse">
                  <RefreshCw className="animate-spin" size={14} /> Gerando
                  Documento...
                </div>
              )}
            </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredReports.map((rep) => {
                    const cfg = reportStatusConfig[rep.status] || {
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-5 font-mono text-[10px] font-black text-blue-900">
                          {rep.id}
                        </td>
                        <td className="px-6 py-5">
                          ...
                          <div className="text-[9px] text-slate-400 font-bold uppercase mt-1">
                            Ref: {rep.activityId} • {rep.province}
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <div className="inline-flex items-center px-2.5 py-1.5 rounded-lg bg-indigo-50 border border-indigo-100 text-indigo-700">
                        ...
                        <td className="px-6 py-5 text-center">
                          <span
                            className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm inline-flex items-center gap-1.5 ${cfg.color}`}
                          >
                            <cfg.icon size={12} /> {rep.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-center">
                          <div className="flex justify-center gap-2">
                            <button
                              onClick={() => openPdfViewer(rep)}
                              className="bg-white border border-slate-200 text-slate-600 hover:text-blue-900 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm flex items-center gap-1.5 disabled:opacity-50"
                            ...
                            <button
                              ...
                              className="bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm flex items-center gap-1.5 disabled:opacity-50"
                              disabled={isGenerating}
                            >
                            ...
                            <button
                              onClick={() =>
                                navigate(
                                  `/atividades/${rep.activityId}/relatorio`,
                                )
                              }
                              className="bg-white border border-slate-200 text-indigo-600 hover:text-indigo-900 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm flex items-center gap-1.5"
                            >
                              <Edit size={12} /> Editar
                            </button>
                          </div>
      ...
      <Suspense fallback={null}>
        ...
        <ConfirmationModal
          ...
          onConfirm={() =>
            setActivities((prev) =>
              prev.filter((a) => a.id !== activityToDelete.id),
            )
          }
          title="Eliminar Atividade"
          message={`Tem a certeza que deseja eliminar "${activityToDelete?.title}"?`}
        />
        <PdfViewerModal
