...
const getPageTitle = (pathname: string): string => {
  ...
  if (pathname.match(/^\/atividades\/.+\/relatorio/)) return 'Gerar Relatório de Missão';
  if (pathname.match(/^\/ordens-de-servico\/editar\/.+/)) return 'Editar Ordem Operativa';
...
const MainLayout = () => {
  ...
  useEffect(() => {
    ...
    if (role === 'coordenador_central') {
        const allowed = ['/gestao-operacional', '/perfil', '/notificacoes', '/dashboard'];
        ...
        if (!path.startsWith('/coordenacao-central') && 
            path !== '/perfil' && 
            path !== '/dashboard' && 
        ...
        const allowed = ['/tecnico-operacao-central', '/relatorios', '/atividades', '/perfil', '/notificacoes', '/dashboard', '/gestao-operacional', '/procedimento-gma', '/ordens-de-servico'];
        const isAllowed = allowed.some(p => path.startsWith(p));
        if (!isAllowed) {
            navigate('/dashboard', { replace: true });
        }
    ...
    if (initialLoadRef.current) {
      ...
      if (location.pathname === '/') {
        ...
        if (role === 'coordenador_central') navigate('/gestao-operacional', { replace: true });
        else if (role === 'coordenador_utl_regional') navigate('/coordenacao-regional', { replace: true });
        else if (role === 'tecnico_operacao_provincial') navigate('/tecnico-operacao-central', { replace: true });
